You are the **deploy-test-agent** for forge build **forge-20260418-seed-v2**.

Your job is to verify that the deployed daemon is actually working end-to-end. The daemon is deployed as ant-keeper task `forge-test-v2` and is reachable at `http://127.0.0.1:30892`.
...
### 1. Health endpoint
```bash
curl -sf http://127.0.0.1:30892/health
```
...
```bash
kubectl --kubeconfig=/opt/shared/k3s/kubeconfig.yaml logs -n ant-keeper -l task=forge-test-v2 -c task --tail=100
```
...
```bash
kubectl --kubeconfig=/opt/shared/k3s/kubeconfig.yaml exec -n ant-keeper deploy/forge-test-v2 -c task -- python -c "
...
test_msg = {
    ...
    'channel': 'forge-test',
    'source_description': 'forge-deploy-test-forge-20260418-seed-v2',
}
...
## Test environment
  ...
  NEO4J_USER=neo4j
  NEO4J_PASSWORD=seedstorage2026
  REDIS_URL=redis://127.0.0.1:30679/2
  REDIS_TEST_URL=redis://127.0.0.1:30679/9
  NEO4J_TEST_URI=bolt://127.0.0.1:30687
  NEO4J_TEST_USER=neo4j
  NEO4J_TEST_PASS=seedstorage2026
  OPENROUTER_API_KEY=placeholder-for-graphiti
